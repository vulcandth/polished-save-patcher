"""Generate human-readable Rust v8→v9 mapping tables from the upstream C++ source.
...
Usage (from repo root):
    python tools/gen_v8_to_v9_maps.py

You can also override paths:
    python tools/gen_v8_to_v9_maps.py --cpp path/to/PatchVersion8to9.cpp \
        --out crates/gb-save-polished/src/migrations/v8_to_v9_maps_data.rs
"""
...
DEFAULT_OUT = (
    ...
    / "src"
    / "migrations"
    / "v8_to_v9_maps_data.rs"
)


@dataclass(frozen=True)
class MapSpec:
    """Describes a single mapping to extract from the upstream C++ file."""
...
SPECS: List[MapSpec] = [
    MapSpec("EVENT_FLAG", "mapV8EventFlagToV9", "u16_u16"),
    MapSpec("KEY_ITEM", "mapV8KeyItemToV9", "u8_u8"),
]


def _find_function_block(cpp: str, func_name: str) -> str:
    # Avoid matching call sites like `mapV8EventFlagToV9(` inside other functions.
    # We want the actual function definition, which starts at the beginning of a line
    # with a return type preceding the function name.
    m = re.search(rf"^[\t ]*\w[\w:<>,\t &*]*\b{re.escape(func_name)}\s*\(", cpp, re.MULTILINE)
    if m is None:
